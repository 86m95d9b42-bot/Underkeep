...
- **Phase 2's "walk to the boss arena" needs doors, which are Phase 7.** A stuck or locked door on the critical path stops the walk in the app: opening one is the Chest / Door screen's job, and the context key says "Not built yet" over it. The walk itself is proven in `test/movement.test.js`, which crosses every floor with the minimum hero's door assumptions. Pull door bashing forward into Phase 2, or accept that the on-phone walkthrough waits for Phase 7? (Default: accept, and test the walk.)
