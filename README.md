...
Those two files are working code from Depths of Dreadmoor. They're vendored essentially as-is; Underkeep builds `floor-builder.js` and `view.js` around them. Their logic is untouched — only their module wrappers changed, because the UMD form cannot load in an ES module project at all (see `docs/DECISIONS.md`).
